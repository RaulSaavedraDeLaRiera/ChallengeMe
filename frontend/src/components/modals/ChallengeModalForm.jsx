...
export const ChallengeModalForm = ({ onSuccess }) => {
  ...
  const minSelectableDate = isoDate(todayStart)
  const [startDate, setStartDate] = useState(isoDate(todayStart))
  ...
  const submit = async () => {
    ...
    const isStartBeforeToday = start < todayStart
    const isEndBeforeToday = end < todayStart
    if (isNaN(start.getTime()) || isNaN(end.getTime())) { setError('Invalid dates'); return }
    if (isStartBeforeToday || isEndBeforeToday) { setError('Dates cannot be before today'); return }
        ...
        <div className={styles.colDate}>
          ...
          <input
            ...
            onChange={(e) => {
              ...
              const clampedValue = value < minSelectableDate ? minSelectableDate : value
              setStartDate(clampedValue)
              if (clampedValue > endDate) setEndDate(clampedValue)
            }}
          />
        </div>
        <div className={styles.colDate}>
          ...
          <input
            ...
            onChange={(e) => {
              const value = e.target.value
              const minEnd = startDate || minSelectableDate
